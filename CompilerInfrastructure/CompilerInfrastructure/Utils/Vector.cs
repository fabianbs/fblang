namespace CompilerInfrastructure.Utils {
    ...
    public struct Vector<T> : IEnumerable<T>, IEquatable<Vector<T>> {
        ...
        public Vector(IEnumerable<T> other) {
            if (other is Vector<T> vec) {
                ...
                switch (other) {
                    case T[] tarr:
                        arr = (T[]) tarr.Clone();
                        capacity = Length = checked((uint) arr.LongLength);
                        break;
                    case ICollection<T> coll: {
                        var cap = Length = checked((uint) coll.Count);
                        capacity = cap;
                        arr = new T[cap];
                        coll.CopyTo(arr, 0);
                        break;
        ...
        void EnsureCapacity(uint minCap) {
            ...
            Array.Resize(ref arr, (int) capacity);
            arrVal = Unsafe.As<StructBox<T>[]>(arr);
        }
        ...
        public T PopBack() {
            if (Length <= 0)
                return default;
            var ret = arrVal[--Length];
            arrVal[Length] = default;
            return ret;
        ...
        public ref T Back() {
            if (Length > 0)
                return ref arrVal[Length - 1].Value;
            throw new IndexOutOfRangeException();
        }
        ...
        public void AddRange(params T[] vals) {
            if (vals is null)
                return;
            EnsureCapacity(checked(Length + (uint) vals.LongLength));
            Array.Copy(vals, 0, Unsafe.As<T[]>(arrVal), Length, vals.LongLength);
            Length += (uint) vals.LongLength;
        }
        ...
        public T[] AsArray() {
            ...
            if (Length == capacity)
                return ret;
            Array.Resize(ref ret, (int) Length);
        ...
        public T[] AsArray(uint offset, uint count) {
            ...
            if (offset > Length)
                return Array.Empty<T>();
            Array.Copy(arrVal, offset, arrVal, 0, Length - offset);
            Length -= offset;
        ...
        public Span<T> ToSpan() {
            return new Span<T>(Unsafe.As<T[]>(arrVal), 0, (int) Length);
        }

        public Span<T> ToSpan(int offset) {
            return new Span<T>(Unsafe.As<T[]>(arrVal), offset, (int) Length - offset);
        }

        public Span<T> ToSpan(uint offset) {
            return new Span<T>(Unsafe.As<T[]>(arrVal), (int) offset, (int) (Length - offset));
        }

        public Span<T> ToSpan(int offset, int len) {
            return new Span<T>(Unsafe.As<T[]>(arrVal), offset, Math.Max(0, Math.Min((int) Length - offset, len)));
        }

        public Span<T> ToSpan(uint offset, uint len) {
            return new Span<T>(Unsafe.As<T[]>(arrVal), (int) offset,
                               (int) (offset + len > Length ? offset > Length ? 0 : Length - offset : len));
        }
        ...
        public override int GetHashCode() {
            return arrVal.AsSpan(0, (int) Length).GetArrayHashCode();
            /*if (arrVal is null)
                return 0;
            else {
